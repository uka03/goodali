...
class _MyCoursesDetailState extends State<MyCoursesDetail> {
  ...
  int type = 0;
  List<CourseLessonsTasks> readingTasks = [];
  List<CourseLessonsTasks> listeningTasks = [];
  List<CourseLessonsTasks> videoTasks = [];
  List<CourseLessonsTasks> allTasks = [];
  ...
  Future<List<CourseLessonsTasks>> getCoursesTasks(String id) async {
    ...
    readingTasks = allTasks
        .where((item) =>
            ...
            item.type == 6)
        .toList();
    listeningTasks = allTasks.where((element) => element.type == 1).toList();
    videoTasks = allTasks.where((element) => element.type == 4).toList();
    print("readingTasks ${readingTasks.length}");
    print("videoTasks ${videoTasks.length}");
    print(tasksName);
    return allTasks;
  }
}
